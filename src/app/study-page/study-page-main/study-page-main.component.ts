...
// ???? ???? META 단어, 일반 단어가 따로 관리해야 할 정도인지? 오히려 분리 함으로써 헷갈리진 않을지?

// 회원가입시 개인정보를 수집, 관리할 것인지? 만약 할 것이면 동의를 받아야 함
// ???? 기타 정보보안 법 관련하여 공부 할 것
...
// ???? 크롬 개발자 도구에서 보안이 정상적으로 동작하는지 확인
...
// 게시글 비밀글 기능 추가할 것
// ???? 권한에 따라 볼 수 있는지 여부를 확인 (서버에서 게시글을 뿌릴때 처리, 화면에서 처리하면 안됨)
// ???? 기본적으로 모두가 볼 수 있게, 하지만 민감한 사항은 숨기도록
// ???? 비밀번호를 걸 것인지? 아니면 애초에 존재 자체를 숨길 것인지? 양쪽 다 가능하게 할 것인지?
// ???? ???? 회원별로 레벨(운영자레벨, 우수회원, 일반회원, 비회원 등)을 정해두고, 게시글 작성 시 볼 수 있는 레벨을 정할 수 있게 하는건? (볼 수 있는 레벨의 최고치는 자신의 레벨로, ex) 일반 회원은 우수 회원 레벨로 설정 불가)

// 권한에 대한 고려 필요
// ???? 운영자의 권한 철저하게 나누어서 관리
// ???? 일반 유저의 권한은 일반 유저가 읽을 수 있는 게시글 열람으로 한정 할 것
// ???? 권한은 숫자(레벨)로 지정하여 융통성 있게 사용할 수 있도록 할 것
// ???? 권한 관련된 것은 반드시 서버단에서 처리할 것(화면에서 처리하면 보안상 문제가 발생할 가능성이 높아짐)
...
// ???? ???? 공통 메시지의 팝업창 구성도 생각할 것
// ???? ???? ???? 에러메시지 : 다국어 처리

// 자유게시판 만들 것인지 구상
// ???? 자유게시판은 화사하게 구상? 유니콘!
// ???? 보는 사람이 즐거움을 느낄 수 있도록 구성
// ???? ???? 동화같은 느낌?
...
// ???? 스타일을 별도로 작성해서 그것을 참조하도록 하면?
// ???? ???? 스타일 자동으로 맞춰주는 툴은 없는지?

// 서버 성능 개선을 위해 어떻게 해야 할 것인지?
// ???? 추가로 개발시 사용될 수 있는 성능 체크 방법을 생각해 볼 것
// ???? ???? 크롬 개발모드 등을 활용하는것도 한 방법.
// ???? 성능 개선 방안 1. 화면-서버, 서버-DB 간의 연결을 최소화 시킬 것
// ???? 성능 개선 방안 2. 서버에서 공통으로 처리하는 모듈들을 묶을 것
// ???? 성능 개선 방안 3. 처리로직 단순화
...
// ???? 5. 간단한 네트워크 보안 프로그램
// ???? ???? 트래픽 분석
// ???? ???? ???? 실시간으로 요청을 분석하여, 크리티컬한 영역에서의 과한 중복 요청은 피하도록 설계
// ???? ???? 보안 관제센터 개념?
// ???? ???? 네트워크(ip 등을 기준으로) 차단을 쉽게 할 수 있도록 돕는 앱?
// ???? 6. 어려운 것을 쉽게 해주는 기능부터 생각해 보자.
// ???? ???? 각종 대출금 계산은 어떤지? (-> 은행 관련)
// ???? ???? 공학 관련 계산은? (-> 탄도학 등)
// ???? ???? 천문학 계산은?
// ???? 7. 정말 쉽게 쓸 수 있는 그래프 라이브러리? (방식 : 라이브러리 호출하고, 데이터만 입력하면 그래프 뿅?)
// ???? 8. API 같은 것은?
...
// Angular2+의 특징, 장점, 단점, 기능 등을 다시 한번 공부 해서 프로젝트 이어 나가자. 10월달에 오픈이 목표
// ====================
// 1. Angular2+의 특징
// ====================
// 컴포넌트 등
